...
case "$EVCC_MODE" in
    tls)
        ...
        ;;
    notls)
        echo "=== [2/2] EcoG iso15118 EVCC (HLC layer, plaintext EIM) on eth0 (proxy_net2) ==="
        exec make run-evcc config=iso15118/shared/examples/evcc/iso15118_2/evcc_config_eim_ac.json
        ;;
    *)
